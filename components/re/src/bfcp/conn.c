...
static void tcp_conn_handler(const struct sa *peer, void *arg)
{
	...
	if (bc->connh) {
		...
		err = re_tcp_accept(&bc->tc, bc->ts, tcp_estab_handler,
		 ...
		 bfcp_recv_h *recvh, bfcp_close_h *closeh, void *arg)
{
	...
	case BFCP_TCP:
		err = re_tcp_connect(&bc->tc, peer, tcp_estab_handler,
			   tcp_recv_handler, tcp_close_handler, bc);
		if (err)
			goto out;
...
int bfcp_accept(struct bfcp_conn *bc)
{
	...
	return re_tcp_accept(&bc->tc, bc->ts, tcp_estab_handler,
		tcp_recv_handler, tcp_close_handler, bc);
}
