...
int net_getifaddrs(net_ifaddr_h *ifh, void *arg)
{
    ...
    netif = esp_netif_next(netif); // 获取第一个网络接口
    while (netif != NULL) {
        if_name = (char *)esp_netif_get_desc(netif);
        if (esp_netif_get_ip_info(netif, &ip_info) == ESP_OK) {
            struct sockaddr_in sa;
            sa.sin_family = AF_INET;
            sa.sin_addr.s_addr = ip_info.ip.addr;

            DEBUG_INFO("ifaddr: %10s\n", if_name);

            if (ifh(if_name, (struct sockaddr_storage *)&sa, arg)) {
                break;
            }
        }
        netif = esp_netif_next(netif); // 获取下一个网络接口
    }
